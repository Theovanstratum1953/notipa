...
8. **PWA install + web push notifications — done.** Installability (manifest + service worker) was already in place from an earlier revision; this pass added the missing half: `core.models.PushSubscription`, `core.push` (VAPID-based send helper, safely a no-op if a school hasn't configured VAPID keys), `core.views.push_subscribe`/`push_unsubscribe`, service-worker `push`/`notificationclick` handlers, and a per-device "App & Notifications" page (Help section) covering both install instructions (one-click on Android/desktop, manual Add to Home Screen on iOS 16.4+) and enabling notifications. Wired into every existing "new content" trigger: Announcements (on publish), Homework, Fee Notices, Permission Slips, Guardian ↔ Teacher and Class Group messages, and Report Cards (on publish, including the bulk-publish action) — Attendance deliberately excluded, since a daily present/absent record isn't the kind of thing that should push a notification. This was the last remaining item from the original Phase 1 build sequence.
...
- **PWA install and web push notifications are no longer in this list** — see Section 9, item 8. `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` are unset by default (see `env.example.txt` and the note on those settings in `notipa/settings.py`); a self-hosting school simply doesn't get push until it generates its own key pair with `python -m py_vapid --gen` and sets them.
